...
## 22. 音声QA —— レシピ改訂と候補カード更新（`eac0cfb`）

| 項目 | 内容 |
|---|---|
| 対象 SHA | `eac0cfbf8a28b4c31061dd8d31f0d67b7b3844e2` |
| 環境 | **Vercel Preview**（Production ではない） |
| 端末 | **iPhone 実機 / Safari** |
| 記録日 | 2026-08-17 |
| 結果 | **B / C / D いずれも PASS** |
...
「レシピを作って」→「顆粒だしではなく昆布とかつお節でだしを取る形に変えて」という
1連の音声依頼。

| 観測項目 | 結果 |
|---|---|
| 候補カードの枚数 | **1枚**（変更前パネルは残っていない） |
| 顆粒だし | **消えた** |
| 昆布・かつお節 | **表示された** |
| 初期チェック | **全件 OFF** |
| 最後まで読み上げ | はい |
...
⚠️ **「カード1枚」の定義**: 複数の候補項目を内包する「買い物候補」パネル**全体**を
1枚と数える。個々の候補行ではない。

### 22.2 トレース上の経路

```
create_recipe → suggest_shopping_items
revise_recipe → suggest_shopping_items（新 recipe_id）
add_inventory_item は呼ばれていない
```

### 22.3 DB 読み取り検証（SELECT のみ）

**元レシピは残り、内容も `updated_at` も不変**

```
id=f5c275bb-093b-4e29-bea8-678b601fef6d
  title=シンプルな卵と玉ねぎの味噌汁
  created=2026-08-16T21:09:08.748743Z
  updated=2026-08-16T21:09:08.748743Z   ← created と同一 = 一度も更新されていない
  ingredients=["卵","玉ねぎ","水","顆粒だし","味噌"]
```

**改訂版はちょうど1件**

```
id=e746748d-5747-4316-a9f9-323c27405b35
  title=シンプルな卵と玉ねぎの味噌汁（昆布とかつお節でだしを取る）
  created=updated=2026-08-16T21:09:54.11492Z
  ingredients=["卵","玉ねぎ","水","昆布","かつお節","味噌"]
```

**顆粒だしは無く、昆布とかつお節が入っている。** その時間帯に作られたレシピは
**2件**（元 + 改訂版）で、重複作成なし。

**台帳** —— `create_recipe` / `suggest_shopping_items` / `revise_recipe` /
`suggest_shopping_items` が**各1回、エラーなし**。`revise_recipe` の結果に
`supersedes_recipe_id=f5c275bb…` が入っており、C の lineage 相関はこの値を使う。

**副作用なし**

| テーブル | 結果 |
|---|---|
| `inventory_items` | **11件**。QA時間帯（21:00以降）の新規行 **0件**。かつお節の行は**存在しない** |
| `shopping_items` | **3件**（じゃがいも / にんじん / じゃがいも）で不変 |
| `cooking_sessions` | 最終更新は 2026-08-16T12:18:45 で、**21:09 のQAより前**。`recipe_snapshot` も不変 |

`recipe_snapshot` が改訂の影響を受けないことは設計どおり
（セッションはスナップショットを正とする。§20 の調査を参照）。

### 22.4 D（rate limit）も PASS

| 観測項目 | 値 |
|---|---|
| R8 | `rate_limit_exceeded` |
| snapshot | `remaining=9427` / `resetIn=46s` |
| cooldown 実測 | **約46.9秒** |
| UIカウントダウン | 機能した |
| cooldown 後の手動再試行 | forced final が完走 |

**`remaining` が 9427（正の値）でも報告された 46秒を採用している。**
これが §20.5 で直した点そのもので、修正前は 2〜2.5秒の backoff に落ちていた。

### 22.5 ⚠️ 未解決 —— レート制限の発生頻度

**cooldown からの復旧は機能するが、そもそも制限に当たる頻度は下がっていない。**
今回も4分程度の会話で1回発生している。
...
- **`revise_recipe` を選ぶかは強制されていない。** A の在庫ゲートと違い、
  revise には fail-closed の構造的強制が無く、今回はモデルが正しく選んだだけ。
  「変更しました」と口頭で言って終わる可能性は残っている
