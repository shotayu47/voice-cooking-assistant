...
**4 行とも 2026-08-08 に適用済み。** 適用前のプロジェクト設定は OTP 長 = 8 桁で、
UI が 6 桁固定のためログインできなかった（実測して判明）。メールテンプレートの
2 行は §11 の順序どおり、コードのデプロイ後に変更した。
...
| 3 | コードをデプロイする（この branch） | ✅ 2026-08-08（`776d0fb` / Production success） |
| 4 | Magic Link / Confirm signup テンプレートを `{{ .Token }}` 本文へ変更 | ✅ 2026-08-08 |
| 5 | 実機で 1 回ログインして確認 | ✅ 2026-08-08（§12「実機確認」） |

1 と 2 を先に済ませても現行ログインは壊れない（Magic Link のリンクは OTP 長と
無関係に機能する）。**壊れるのは 4 だけ**なので、4 は 3 の後に行った。

`?token_hash=` 経路は残してあるので、4 の直前までに送信済みのメールは
有効期限内であれば引き続き使えた。この経路は移行期間のためのもので、
有効期限内の旧メールが尽きれば削除してよい（§13）。

### Email OTP 移行 — **COMPLETE**（2026-08-08）

手順 1〜5 がすべて完了し、本番の実機で成立を確認した。
**TSUGU の正式なログイン方式は Email OTP（6桁コード）である。**
Magic Link は UI から撤去済み。`/auth/callback` は将来の OAuth のために残してある。

残っているのは §13 のフォローアップだけで、いずれも**この移行の完了条件ではない**。
...
### 実機確認（2026-08-08 / iPhone）

上の UI 通しは `npm run dev:otp` でコードを取得しており、**メール配送そのものは
経由していない**。テンプレート変更（§11 手順 4）後、本番環境で実際にメールを
受け取る経路を実機で確認した。

| 項目 | 結果 |
| --- | --- |
| iPhone 実機で 6 桁コードを受信 | ✅ |
| ホーム画面 PWA 上でコードを入力してログイン成功 | ✅ |
| ログイン後も既存の在庫・調理履歴が維持されている | ✅ |

**§1 で挙げた失敗経路が本番で消えたことの確認になっている。** メールにリンクが
無いため、メールアプリ内蔵ブラウザ → Safari の遷移が発生せず、コード入力から
セッション確立までが PWA の中で完結する。既存データが見えていることは、
`auth.users.id` が移行前後で同じであることを利用者側から見た形での裏づけでもある。

---

## 13. スコープ外 / 未完了のフォローアップ

Email OTP 移行（§11）は COMPLETE。以下は**いずれもその完了条件ではなく**、
別件として残っている。

| 項目 | 状態 | 備考 |
| --- | --- | --- |
| Custom SMTP（Resend） | **未完了** | 内蔵 SMTP は 1 時間あたり実測 2 通・**組織メンバーの登録済みアドレスにしか届かない**（§10）。**他人にアカウントを配るには必須** |
| 独自ドメイン取得 | **未完了** | Resend はドメイン未検証だと自分の登録アドレス宛にしか送れない。Custom SMTP の前提条件。SPF / DKIM / DMARC の設定を伴う |
| Google Sign-In | **未完了** | 次の認証フェーズ。`?code=` 経路は今回維持済みなので追加しやすい。検証済みメール一致で `auth.users.id` は自動リンクにより維持される |
| Apple Sign-In | 保留 | Apple Developer Program（$99/年）が必要。Hide My Email は別 `user.id` を作りうるため、導入時は `linkIdentity()` 経由に寄せる |
| Passkey | 保留 | Supabase 側が experimental。RP ID を変えると既存 passkey が全て無効になるため、独自ドメイン確定後に再評価する |
| Cookie の `httpOnly` 変更 | 今回は変更しない | 上げるとクライアントから `linkIdentity()` を呼べなくなる。Google 連携の方式決定後に判断する |
| `?token_hash=` 経路の削除 | 保留 | 移行期間用（§11）。有効期限内の旧 Magic Link メールが尽きた後に削除してよい |

**Supabase 内蔵 SMTP を使用している現状では、認証メールの送信先は Supabase 組織の
チームメンバーとして事前承認されたメールアドレスに限定される。一般ユーザーへの
提供には Custom SMTP の設定が必要。** これは実装の欠落ではなく内蔵 SMTP の制約で
あり、解消には上の 1〜2 行目が要る。
