...
get_inventory を全件走査するより、こちらで名前を指定する方が確実。`,
  ...
  },
  {
    ...
    function: {
      ...
      description:
        `**すでに手元にある**食材を在庫に登録する。名前だけで登録可能。数量が不明なら quantity を null にすること。
「在庫に追加して」「買ってきたので入れて」「家にある」など、**所有している**と分かる場合だけ呼ぶこと。
「買い物候補に入れて」「買い物リストに追加して」「買うものに入れて」「〜が必要」は**買いたい**という意味で、
在庫ではない。これらでは呼ばないこと。買い物候補は suggest_shopping_items がレシピの材料から作る。`,
      parameters: {
        type: 'object',
        properties: {
...
max_from_inventory の capacity_status が exact でない場合、「最大◯人分」と断定しないこと。`,
  ...
  },
  {
    ...
    function: {
      ...
      description:
        `現在の工程を取得する。工程は進まない。
「次の工程を教えて」「次なに？」「この次は何をする？」「今どこ？」「現在の工程は？」は
すべてこのツールで答えること。次の工程を"案内する"のはこちらであって、工程を進めることではない。
時間・火力・分量・手順を聞かれたときもこれを使うこと。`,
      parameters: {
        type: 'object',
  ...
  },
  {
    ...
    function: {
      ...
      description:
        `工程を1つ進め、完了として記録する（データベースを更新する）。
ユーザーが完了・前進を明示した場合のみ呼ぶこと。例:「できた」「終わった」「この工程は完了」
「次へ進めて」「この工程を完了にして」。
質問には呼ばない。とくに「次の工程を教えて」「次なに？」は"案内"の依頼であって完了の合図ではない。
これらは get_current_cooking_step を使うこと。
単独の「次」だけでは判断できないので呼ばず、完了かどうかを確認すること。
